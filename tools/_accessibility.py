    ...
    with _lock:
        ...
        try:
            if not (_BIN.exists() and _BIN.stat().st_mtime >= _SRC.stat().st_mtime):
                subprocess.run(["swiftc", "-O", str(_SRC), "-o", str(_BIN), "-module-cache-path", str(Path(tempfile.gettempdir()) / "endeavor-swift-module-cache"), "-framework", "AppKit", "-framework", "ApplicationServices"], check=True, capture_output=True, timeout=120)
...
def read_frontmost(max_nodes: int = 220) -> dict:
    ...
    try:
        r=subprocess.run([str(_BIN),str(max(1,min(int(max_nodes),500)))],capture_output=True,text=True,timeout=8)
